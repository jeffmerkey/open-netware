...
long memcmp(const char *s1, const char *s2, LONG n)
{
    register LONG i;
...
long printf(char *format, ...)
{
	...
	retCode = ScreenTTYOut(consoleScreen, buffer, strlen(buffer), consoleScreen->NormVid);
	va_end(parms);

	return retCode;
}

long printfScreen(SCREEN *screen, char *format, ...)
{
	va_list parms;
	long retCode;
	BYTE buffer[512];
	...
	va_start(parms, format);
	_fmt(format, &parms, (char *)buffer);
	retCode = ScreenTTYOut(screen, buffer, strlen(buffer), screen->NormVid);
	va_end(parms);

	return retCode;
}

long ConsolePrintf(char *format, ...)
{
	va_list parms;
	long retCode;
	BYTE buffer[512];
...
void sprintf_comma(char *buf, char *fmt, long value)
{
...
long printfWithAttribute(LONG attr, char *format, ...)
{
	va_list parms;
	long retCode;
	BYTE buffer[512];
	...
	va_start(parms, format);
	_fmt(format, &parms, (char *)buffer);
	retCode = ScreenTTYOut(GetVidOwner(), buffer, strlen(buffer), attr);
	va_end(parms);

	return retCode;
}

long printfScreenWithAttribute(SCREEN *screen, LONG attr, char *format, ...)
{
	va_list parms;
	long retCode;
	BYTE buffer[512];
	...
	va_start(parms, format);
	_fmt(format, &parms, (char *)buffer);
	retCode = ScreenTTYOut(screen, buffer, strlen(buffer), attr);
	va_end(parms);
